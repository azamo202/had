    ...
    <header className="topbar">
      ...
      <div ref={ref} style={{ display: 'flex', gap: 14 }}>
        <div style={{ position: 'relative' }}>
          ...
          {notifOpen && (
            <div className="card" style={{ position: 'absolute', top: 50, insetInlineEnd: 0, width: 340, boxShadow: 'var(--shadow-lg)', zIndex: 60, padding: 0, overflow: 'hidden' }}>
              ...
              <div style={{ maxHeight: 360, overflow: 'auto' }}>
                {db.notifications.length === 0 ? (
                  <div style={{ padding: 20, textAlign: 'center', color: 'var(--text-3)', fontSize: 13 }}>لا توجد إشعارات</div>
                ) : (
                  db.notifications.slice(0, 8).map((n) => {
                    const s = st(n.tone || 'on_track');
                    return (
                      <div key={n.id} onClick={() => { dispatch({ type: 'NOTIF_READ', id: n.id }); setNotifOpen(false); nav('/notifications'); }}
                        ...
                        <div style={{ minWidth: 0 }}>
                          ...
                          <div className="muted" style={{ fontSize: 11.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{n.body || n.entity}</div>
        ...
        <div style={{ position: 'relative' }}>
          ...
          {userOpen && (
            <div className="card" style={{ position: 'absolute', top: 50, insetInlineEnd: 0, width: 220, boxShadow: 'var(--shadow-lg)', zIndex: 60, padding: 8 }}>
